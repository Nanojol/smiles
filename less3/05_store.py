...
for product in goods:
    ...
    total_count = 0
    total_price = 0
    for type in store[goods[product]]:
        # print(type)
        count_product = type['quantity']
        price_product = type['price']
        total_count += count_product
        full_price = count_product * price_product
        total_price += full_price
    print(product, '-', total_count, 'шт, стоимость', total_price, 'руб')

# print(store[goods[product]][0]['quantity'])
# print(product, '-', store[goods[product]}, 'шт, стоимость', <общая стоимость>, 'руб')
